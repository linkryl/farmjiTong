...
bool BackpackLayer::addItemUI(const std::string& itemImagePath, int i, int id)
{
    ...
    float x = 370 + (currentItems % 12) * 51; // 每行 12 个物品，间隔 51
    float y = 572 - (currentItems / 12) * 55; // 每列间隔 55
    itemButton->setPosition(Vec2(x, y));

    itemButton->addClickEventListener([this, id, x, y](Ref* sender)
        {
            // 加载点击音效
            audioPlayer("../Resources/Music/ClickSoundEffect.mp3", false);
            syncBagContentsToUI();
            ...
            // 添加按钮点击事件
            sellButton->addClickEventListener([this, id, popupLayer](Ref* sender)
                {
                    // 加载点击音效
                    audioPlayer("../Resources/Music/ClickSoundEffect.mp3", false);
                    Bag::getInstance()->removeItem(id, 1);
                    Bag::getInstance()->money += 10;
                    popupLayer->removeFromParentAndCleanup(true);
                    syncBagContentsToUI();
                });
            // 将按钮添加到小窗口
